...
class Renderer():
    ...
    def reset_bbox(self,):
        ...
        bbox[0, 2] = torch.tensor(self.width).float().to(self.device)
        bbox[0, 3] = torch.tensor(self.height).float().to(self.device)
        self.bboxes = bbox
    ...
    def render_with_ground_multimesh(self, verts1, verts2, faces, colors1, colors2, cameras, lights):
        """
        :param verts (B, V, 3)
        :param faces (F, 3)
        :param colors (B, 3)
        ...
        # (B, V, 3), (B, F, 3), (B, V, 3)
        verts1, faces, colors1 = prep_shared_geometry(verts1, faces, colors1)
        
        B, V, _ = verts2.shape
        colors2 = colors2.unsqueeze(1).expand(B, V, -1)[..., :3]

        # (V, 3), (F, 3), (V, 3)
        gv, gf, gc = self.ground_geometry
        verts1 = list(torch.unbind(verts1, dim=0)) + [gv]
        faces = list(torch.unbind(faces, dim=0)) + [gf]
        colors1 = list(torch.unbind(colors1, dim=0)) + [gc[..., :3]]
        mesh1 = create_meshes(verts1, faces, colors1)

        verts2 = list(torch.unbind(verts2, dim=0)) + [gv]
        colors2 = list(torch.unbind(colors2, dim=0)) + [gc[..., :3]]
        mesh2 = create_meshes(verts2, faces, colors2)
        ...
        results = self.renderer(mesh1, cameras=cameras, lights=lights, materials=materials) + self.renderer(mesh2, cameras=cameras, lights=lights, materials=materials)
        results[0, ..., :3] /= 2
        image = (results[0, ..., :3].cpu().numpy() * 255).astype(np.uint8)
