...
class Encoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, batch):
        x, y, mask = batch["coco"], batch["y"], batch["mask"]
        B, J, d, T = x.shape
        ...
        muandsigmaMask = torch.ones((B, 2), dtype=bool, device=x.device)
        ...
        return {"mu": mu, "motion_feat": final[2:]}
...
class Decoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, batch, use_text_emb=False):
        ...
        output[~mask.T] = 0
        output = output.permute(1, 2, 3, 0) # [T, B, J, 3] => [B, J, 3, T]
