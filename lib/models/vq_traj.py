...
class Network(nn.Module):
    def __init__(self, cfg) :
        ...
        self.decoder = Decoder(input_emb_width = 512, output_emb_width = 512, down_t = 3, 
                               stride_t = 2, width = 512, depth = 3, dilation_growth_rate = 3, activation='relu', norm=None)
    ...
    def init_batch(self, batch):
        ...
        if 'w_transl' in data :
            ...
            w_orient_q_tq = w_orient_q.transpose(0, 1).contiguous()         # [T, B, 1, 4]
            w_transl_tq = w_transl.transpose(0, 1).contiguous()             # [T, B, 1, 3]
            w_orient_6d_tq = w_orient_6d.transpose(0, 1).contiguous()       # [T, B, 1, 6]
            local_traj_tp = traj_global2local_heading(w_transl_tq, w_orient_q_tq, local_orient_type=self.local_orient_type)  # [T, B, 11]

            batch['w_orient_6d_tq'] = w_orient_6d_tq
            batch['w_orient_q_tq'] = w_orient_q_tq
